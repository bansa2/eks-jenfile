...
resource "aws_security_group_rule" "eks-cluster-ingress-traffic" {
  ...
  from_port         = 0
  protocol          = -1
  security_group_id = [aws_security_group.eks-cluster-grp.vpc_config[0].cluster_security_group_id ]
  to_port           = 0
  type              = "ingress"
...
resource "aws_security_group_rule" "eks-cluster-node-ingress-cluster" {
  ...
  protocol                 = "tcp"
  security_group_id        = aws_security_group.eks-cluster-node-sg.id
  source_security_group_id = aws_security_group.eks-cluster-grp.id
  to_port                  = 65535
  type                     = "ingress"
}
